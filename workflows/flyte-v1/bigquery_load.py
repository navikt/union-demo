...
import os
import sys
from custom_types import bigquery_table
...
def oracle_to_bq(oracle_table_uri: str, bq_table_uri: str) -> bigquery_table.CustomBigQueryTable:
    ...
    return bigquery_table.CustomBigQueryTable(
       df=df, 
...
def bq_to_pandas(bq_table: bigquery_table.CustomBigQueryTable) -> pd.DataFrame:
   print(bq_table.dataframe)
   return bq_table.dataframe


@workflow
def structured_dataset_bigquery() -> str:
    bq_table = oracle_to_bq(ORACLE_TABLE_NAME, BQ_TABLE_URI)
    df = bq_to_pandas(bq_table)
